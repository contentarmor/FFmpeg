...
OUT_DEBS_DIR="./debs"
FFMPEG_VERSION="4.3"
FFMPEG_SRC_DELIVERY="./ca-ffmpeg-${FFMPEG_VERSION}_${PKG_VERS}"
...
if [ "$COMPILER" = "icpc" ];then
  ...
  LD=icc
  AR=xiar
  PKG_NAME="libffmpeg${FFMPEG_VERSION}i-ca"
else
  CC=gcc
  CXX=g++
  LD=g++
...
function create_deb_lib_delivery
{
    ...
    dh_make --native --library -y -p ${PKG_NAME}_${PKG_VERS} --copyright=lgpl2
    ASSERT_OK ${LINENO}
...
function create_rpm_lib_delivery
{
    ...
    rm -rf ${RPM_BUILDROOT}/${CONTENT_ARMOR_HOME}/share
    ...
    CONFIGURE_COMMAND=$(echo ${CONFIGURE_COMMAND} | sed "s#--prefix=.*ffmpeg_out#--prefix=${CONTENT_ARMOR_HOME}#")
    # Create the list of substitution
    VARS=$(grep -oh '\#<[^>]\+>\#' rpmbuild/SPECS/${PKG_NAME}.spec ${RPM_BUILDROOT}/usr/share/doc/libffmpeg${FFMPEG_VERSION}-ca/* | sort -u | \sed 's/\#<\([^>]\+\)>\#.*/\1/')
    VAR_SUBS="$(for var in ${VARS}; do eval "val=\$${var}"; echo -n "s%\#<${var}>\#%${val}%g;"; done)"
